...
int main(int argc, char** argv)
{
	...
	std::normal_distribution<double> n(3, 3);
	double scale = 1.0;
	Mat img;
	...
	int frame_index = initial_frame_index;
	bool cheat = false;
	...
	while (capture.read(img))
	{
		...
		while (std::getline(position_file, line))
		{
			...
			{
				if (line.substr(0,4) != "car:" && line.substr(0,4) != "pers")
				{
					continue;
				}
				else if (line.substr(0, 4) == "pers")
				{
					line = "person: " + std::to_string((atoi(line.substr(8, 2).c_str()) + 100) / 2) + "%";
				}
